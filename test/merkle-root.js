...
describe("Merkle Trees", function () {
    it("Should be able to verify if address is in whitelist or not", async function () {
        ...
        const list = [
            encodeLeaf(await testAddresses[0].getAddress(), 2),
            encodeLeaf(await testAddresses[1].getAddress(), 2),
            encodeLeaf(await testAddresses[2].getAddress(), 2),
            encodeLeaf(await testAddresses[3].getAddress(), 2),
            encodeLeaf(await testAddresses[4].getAddress(), 2),
            encodeLeaf(await testAddresses[5].getAddress(), 2),
        ];
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const connectedWhitelist = await Whitelist.connect(
                testAddresses[i]
            );
            ...
            const verified = await connectedWhitelist.checkInWhitelist(
                proof,
                2
            );
